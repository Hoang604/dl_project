...
if __name__ == '__main__':
    ...
    model_opt['loss'] = {'type': opt['train.loss'],
                         'margin':  opt['train.margin']}
    if opt['train.loss'] != 'triplet':
        raise ValueError(
            "Loss {} not supported in best mechanism".format(opt['train.loss']))
    ...
    # prepare datasets (supported:  'googlespeechcommand' , 'MSWC', 'MSWCFolders')
    if dataset == 'googlespeechcommand':
        from data.GSCSpeechData import GSCSpeechDataset
        ds_tr = GSCSpeechDataset(data_dir, train_task,
        ...
        ds_tr = MSWCFoldersDataset(data_dir, train_task,
                                   opt['data.cuda'], speech_args)
    else:
    ...
    print("The training task {} of the {} Dataset has {} classes".format(
        dataset, train_task, num_classes_tr))
    # clamp n_way based on availbale classes
    ...
    # import stats
    meters = {'train': {field: AverageValueMeter()
    ...
    scheduler = lr_scheduler.StepLR(
        optimizer, opt['train.decay_every'], gamma=0.5)
    ...
    while epoch < max_epoch and not stop:
        ...
        for samples in tqdm(episodic_loader, desc="Epoch {:d} train".format(epoch + 1)):
            ...
            if (ep_idx) % 10 == 0:
                ...
                checkpoint_file_tmp = os.path.join(
                    opt['log.exp_dir'], 'checkpoint_tmp.pt')
                while stored_ckpt is False:
                    ...
                    try:
                        torch.load(checkpoint_file_tmp, weights_only=True)
                    except EOFError:
        ...
        print("Epoch {:02d}: {:s}".format(
            epoch, log_utils.render_meter_values(meter_vals)))
        ...
        if (epoch + 1) % 10 == 0:
            torch.save(model, os.path.join(
                opt['log.exp_dir'], f'model_epoch_{epoch+1}.pt'))
