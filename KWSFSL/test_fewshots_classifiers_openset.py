...
from utils import filter_opt, AverageValueMeter
import torch
import os
import json
...
os.environ['CUDA_VISIBLE_DEVICES'] = os.environ.get(
    '_CONDOR_AssignedGPUs', 'CUDA0').replace('CUDA', '')
...
def test_model(data_loader, classifier, unknow_id, force_unk_testdata=False):
    ...
    for sample in tqdm(data_loader):
        ...
        p_y, target_ids = classifier.evaluate_batch(
...
#        print(p_y.size())
        if '_unknown_' in classifier.word_to_index.keys():
            ...
            y_pred_close = p_y[(1 - torch.nn.functional.one_hot(unknow_lab,
                                p_y.shape[1])).bool()].reshape(p_y.shape[0], -1)
        else:
            y_pred_close = p_y
            y_pred_ood = None
...
if __name__ == '__main__':
    ...
    if opt['fsl.classifier'] == 'ncm':
        ...
        raise ValueError(
            "Classifier {} is not valid for best mechanism".format(opt['fsl.classifier']))
    ...
    if dataset == 'googlespeechcommand':
        ...
        ds = GSCSpeechDataset(data_dir, pos_task,
        ...
        if neg_task is not None:
            ...
            print("The task {} is used for negative samples".format(
                neg_task))
    elif dataset == 'MSWCFolders':
        from data.MSWCFoldersData import MSWCFoldersDataset
        ds = MSWCFoldersDataset(data_dir, pos_task,
                                opt['data.cuda'], speech_args)
        num_classes = ds.num_classes()
        opt['model.num_classes'] = num_classes
        print("The task {} of the {} Dataset has {} classes".format(
        ...
        ds_neg = None
        if neg_task is not None:
            ds_neg = MSWCFoldersDataset(data_dir, neg_task,
                                        opt['data.cuda'], speech_args)
            print("The task {} is used for negative samples".format(
                neg_task))
    else:
    ...
    train_episodic_loader = ds.get_episodic_dataloader(
        'training', n_way, n_support, n_episodes, sampler=sampler)

    # Postprocess arguments
    #   list of log variables. may be turned into a configurable list usign opt['log.fields'] as
    #   opt['log.fields'] = opt['log.fields'].split(',')
    opt['log.fields'] = ['aucROC', 'accuracy_pos',
                         'accuracy_neg', 'acc_prec95', 'frr_prec95']
    ...
    for ep, support_sample in enumerate(train_episodic_loader):
        ...
        y_score_pos, y_pred_pos, y_true_pos, y_pred_close_pos, y_pred_ood_pos = test_model(
            query_loader, classifier, unk_idx)

        # test on the negative dataset (_unknown_) if present
        if ds_neg is not None:
            neg_loader = ds_neg.get_iid_dataloader(
                'testing', opt['fsl.test.batch_size'])
            y_score_neg, y_pred_neg, y_true_neg, y_pred_close_neg, y_pred_ood_neg = test_model(
                neg_loader, classifier, unk_idx, force_unk_testdata=True)
        else:
    ...
    output_log_file = 'evalGSC_fsl_{}_{}_{}_{}_{}_{}'.format(
        opt['fsl.classifier'], fsl_z_norm, task, n_way, n_support, n_episodes)
    output_file = os.path.join(os.path.dirname(
        opt['model.model_path']), output_log_file)
